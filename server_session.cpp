...
ServerSession::ServerSession(unsigned int localPort, const char* keyFilename, char* password, const char* certFilename, const char* rootCaFilename, const char* peerIdentity) :Session(keyFilename, password, certFilename, rootCaFilename, peerIdentity)
{
    ...
    {
        std::cerr << "Error: Binding Local Address Failed." << std::endl; // Handle Error: Binding Local Address
        _state = UNINITIALIZED_SESSION_STATE;
        return;
    }
...
Session::ReceiveResult ServerSession::receiveMessage(BYTE* buffer, size_t bufferSize, unsigned int timeout_sec, BYTE** ppPayload, size_t* pPayloadSize, ServerSession** ppChildSession, unsigned int* pChildSessionId)
{
    ...
    if (header->sessionId == 0) // new session
    {
        ...
        {
            std::cerr << "Error: Message Type Mismatch with Session ID of 0." << std::endl; // Handle Error: Message Type Mismatch
            return RR_BAD_MESSAGE;
        }
        ...
        // Store the Received DH Public Key
        ...
        if (message2 == NULL)
        {
            std::cerr << "Error: Prepare Sigma Message 2 Failed." << std::endl; // Handle Error: Prepare Sigma Message
            delete newSession;
            return RR_FATAL_ERROR;
        }
        //
        ...
        {
            std::cerr << "Error: Sending Response to New Session Failed." << std::endl; // Handle Error: Sending Response to New Session
        ...
        std::cout << "New Session " << newSession->_sessionId << " Created with " << (newSession->_expectedRemoteIdentityString != NULL ? newSession->_expectedRemoteIdentityString : "a Valid Peer") << std::endl;
    ...
    else // existing session
    {
        ...
        if (it != _activeSessions.end())
        {
            ...
            if (!pSession->active())
            {
                std::cerr << "Error: Received Message for Non-Active Session." << std::endl; // Handle Error: Received Message for Non-Active Session
                return RR_BAD_MESSAGE;
            }

            if (header->messageCounter != pSession->_incomingMessageCounter)
            {
                std::cerr << "Error: Message Counter Mismatch." << std::endl; // Handle Error: Message Counter Mismatch
                return RR_BAD_MESSAGE;
            }
            ...
            case GOODBYE_SESSION_MESSAGE:
            {
                ...
                std::cout << "Session Close Request Received, Closing Session " << pSession->_sessionId << std::endl;
            ...
            case HELLO_DONE_SESSION_MESSAGE:
                if (pSession->_state == HELLO_BACK_SESSION_MESSAGE)
                {
                    ...
                    {
                        std::cerr << "Error: Session Crypto Error, Closing Session " << pSession->_sessionId << std::endl; // Handle Error: Verify Sigma Message
        ...
        {
            std::cerr << "Error: No Active Session Found for Received Message." << std::endl; // Handle Error: No Active Session Found for Received Message
            return RR_BAD_MESSAGE;
        }
    }
